...
- **Database**: `MySQL` is used for the microservices in this Docker stack. Each service uses `sequelize` as the ORM.
...
- `user-service`, `product-service`, `shopping-cart-service`, `order-service`, `payment-service`, `notification-service`, and `nginx`

Use these credentials in phpMyAdmin:
- Host: `mysql`
- User: `root`
- Password: `password`

Here `--build` is used to rebuild the images if there are any changes in the code.
